document.addEventListener('DOMContentLoaded', function() {
    ...
    const organizeCoursesModal = new bootstrap.Modal(document.getElementById('organizeCoursesModal'));
    
    // Elimina o comenta este bloque de verificación de autenticación
    ...
    // Abrir modal para organizar cursos
    document.getElementById('openOrganizeCoursesModal').addEventListener('click', function() {
        loadCourses();
        organizeCoursesModal.show();
    });

    // Función para cargar cursos guardados
    function loadCourses() {
        const savedCourses = JSON.parse(localStorage.getItem('userCourses')) || [];
        const coursesList = document.getElementById('coursesList');
        coursesList.innerHTML = '';
        
        const savedEmail = localStorage.getItem('courseEmail') || 'No se ha guardado ningún correo aún.';
        document.getElementById('courseEmailInput').value = savedEmail;
        document.getElementById('savedCourseEmail').textContent = savedEmail;
        ...
        savedCourses.forEach((course, index) => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-sm btn-danger delete-course-btn" data-index="${index}">
        ...
        const coursesSummaryList = document.getElementById('coursesSummaryList');
        coursesSummaryList.innerHTML = '';
        ...
        savedCourses.forEach((course, index) => {
            ...
            `;
            coursesSummaryList.appendChild(row);
        });
    ...
    document.getElementById('addCourseBtn').addEventListener('click', function() {
        const email = document.getElementById('courseEmailInput').value.trim();
        const name = document.getElementById('courseNameInput').value.trim();
        const section = document.getElementById('courseSectionInput').value;
        
        if (email && name) {
            // Guardar el correo
            localStorage.setItem('courseEmail', email);
            document.getElementById('savedCourseEmail').textContent = email;
            
            // Guardar el curso
            const savedCourses = JSON.parse(localStorage.getItem('userCourses')) || [];
            savedCourses.push({ name, section });
            localStorage.setItem('userCourses', JSON.stringify(savedCourses));
            ...
            notification.className = 'alert alert-custom position-fixed bottom-0 end-0 m-3';
            notification.style.zIndex = '1000';
            notification.innerHTML = '<strong><i class="fas fa-check-circle me-2"></i>Curso guardado</strong>';
            document.body.appendChild(notification);
    ...
    function deleteCourse(index) {
        if (confirm('¿Estás seguro de que quieres eliminar este curso?')) {
            ...
            notification.className = 'alert alert-custom position-fixed bottom-0 end-0 m-3';
            notification.style.zIndex = '1000';
            notification.innerHTML = '<strong><i class="fas fa-check-circle me-2"></i>Curso eliminado</strong>';
            document.body.appendChild(notification);
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            // Cerrar sidebar en móviles al seleccionar una opción
    ...
    const toolsData = [
        ...
        { name: 'Ejercicios Inglés Online', icon: 'fas fa-globe-europe', url: 'https://www.ejerciciosinglesonline.com/buscador', category: 'ingles' },
        { name: 'Convertidor GitHub a RAW', icon: 'fas fa-code-branch', url: 'https://emiliocolor.com/github_a_raw', category: 'desarrollo' },
        { name: 'Selector de imágenes de perros', icon: 'fas fa-dog', url: 'https://emiliocolor.com/selector_de_fondos', category: 'diseno' },
        { name: 'Pixabay (imágenes libres)', icon: 'fas fa-images', url: 'https://pixabay.com/es/', category: 'diseno' },
        { name: 'Pexels (imágenes libres)', icon: 'fas fa-images', url: 'https://www.pexels.com/es-es/', category: 'diseno' }
    ];
    ...
    let selectedShortcuts = JSON.parse(localStorage.getItem('selectedShortcuts')) || [];
    ...
    function renderShortcutsSelection() {
        ...
        toolsData.forEach(tool => {
            const isSelected = selectedShortcuts.some(selected => selected.name === tool.name);
            const item = document.createElement('div');
            item.className = `custom-quick-access-item ${isSelected ? 'selected' : ''}`;
            item.innerHTML = `
                <i class="${tool.icon} me-2"></i>
    ...
    document.getElementById('save-shortcuts-selection').addEventListener('click', () => {
        ...
        selectedShortcuts = selectedItems.slice(0, 3).map(item => {
            const toolName = item.querySelector('span').textContent;
            return toolsData.find(tool => tool.name === toolName);
        });
